  ...
  render() {
    ...
    return (
      <div style={{ padding: 20 }}>
        ...
        <div style={{ margin: 10 }}>
          <button style={{ margin: 10 }} onClick={() => model.add()}>
            New Todo
          </button>
          <button style={{ margin: 10 }} onClick={() => model.load()}>
            Reload Todos
          </button>
          <button style={{ margin: 10 }} onClick={() => model.save()}>
            Save Todos
          </button>
        </div>
        {model.todos.map((todo, index) => (
          <TodoDetail key={todo.id} model={model} todo={todo} index={index} />
        ))}
        <br />
        <TodosMeta />
      </div>
    ...
    return (
      <div>
        ...
        <input
          ...
          type="checkbox"
          checked={todo.isDone}
          onChange={e => model.toggleTodo(todo)}
        />
        <input
          size={200}
        ...
        />
        <button style={{ margin: 5 }} onClick={() => model.remove(todo)}>
          Delete
        </button>
      </div>
    );
